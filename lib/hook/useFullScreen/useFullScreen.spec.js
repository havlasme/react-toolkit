...
describe('useFullScreen', () => {
  ...
  it('should return a memoized requestFullScreen callback', () => {
    const {result, rerender} = setUp(true)
    const [, , requestFullScreen] = result.current
    rerender()
    expect(result.current[2]).toBe(requestFullScreen)
  })

  it('should return a memoized exitFullScreen callback', () => {
    const {result, rerender} = setUp(true)
    const [, , , exitFullScreen] = result.current
    rerender()
    expect(result.current[3]).toBe(exitFullScreen)
  })
})
